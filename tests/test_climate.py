...
class TestSleepmeClimate:
    ...
    def test_initialization(self, mock_coordinator):
        ...
        assert entity._name == "Test Bed"
        assert entity._unique_id == "device_123_climate"
        assert entity._attr_unique_id == f"{DOMAIN}_device_123_thermostat"
        assert entity._state is True  # thermal_control_status == "active"
        assert entity._target_temperature == 72.0
        assert entity._current_temperature == 74.0
    ...
    def test_device_info(self, climate_entity):
        ...
        assert device_info["name"] == "Test Bed"
        assert device_info["manufacturer"] == "SleepMe"
        assert device_info["model"] == "DP999NA"
        assert device_info["sw_version"] == "5.39.2134"
        assert device_info["connections"] == {("mac", "b4:8a:0a:4f:90:54")}
    ...
    def test_current_temperature_missing_data(self, mock_coordinator):
    ...
    def test_current_temperature_key_error(self, mock_coordinator):
        ...
        # The current implementation doesn't handle KeyError properly in the error logging
    ...
    def test_available_connected(self, climate_entity):
        """Test available property when device is connected."""
        assert climate_entity.available is True

    def test_available_disconnected(self, mock_coordinator):
    ...
    def test_available_missing_status(self, mock_coordinator):
    ...
    def test_hvac_mode_missing_control(self, mock_coordinator):
    ...
    def test_hvac_mode_key_error(self, mock_coordinator):
    ...
    def test_preset_mode_none_when_off(self, mock_coordinator):
    ...
    def test_preset_mode_max_cool(self, mock_coordinator):
    ...
    def test_preset_mode_max_heat(self, mock_coordinator):
    ...
    def test_preset_mode_none_normal_temp(self, climate_entity):
        """Test preset mode when temperature is normal."""
        assert climate_entity.preset_mode == PRESET_NONE

    @pytest.mark.asyncio
    async def test_async_set_temperature(self, climate_entity):
        ...
        with patch.object(climate_entity, "async_write_ha_state") as mock_write:
            ...
            assert climate_entity._target_temperature == 75
            mock_write.assert_called_once()

    @pytest.mark.asyncio
    async def test_async_set_temperature_none(self, climate_entity):
        """Test setting temperature with None value."""
        original_temp = climate_entity._target_temperature

        with patch.object(climate_entity, "async_write_ha_state") as mock_write:
            await climate_entity.async_set_temperature(temperature=None)

            assert climate_entity._target_temperature == original_temp
            mock_write.assert_not_called()

    @pytest.mark.asyncio
    async def test_async_set_temperature_float(self, climate_entity):
        """Test setting temperature with float value."""
        with patch.object(climate_entity, "async_write_ha_state") as mock_write:
            await climate_entity.async_set_temperature(temperature=73.5)

            assert climate_entity._target_temperature == 73
            mock_write.assert_called_once()

    @pytest.mark.asyncio
    async def test_async_set_hvac_mode_heat_cool(self, climate_entity):
        """Test setting HVAC mode to heat/cool."""
        with patch.object(climate_entity, "async_write_ha_state") as mock_write:
            await climate_entity.async_set_hvac_mode(HVACMode.HEAT_COOL)

            assert climate_entity._state == HVACMode.HEAT_COOL
            mock_write.assert_called_once()
    ...
    async def test_async_set_hvac_mode_off(self, climate_entity):
        ...
        with patch.object(climate_entity, "async_write_ha_state") as mock_write:
            ...
            assert climate_entity._state == HVACMode.OFF
            mock_write.assert_called_once()

    @pytest.mark.asyncio
    async def test_async_update(self, climate_entity):
        ...
        with patch.object(
            ...
            assert climate_entity._state is True
            assert climate_entity._target_temperature == 72.0
            assert climate_entity._current_temperature == 74.0

    def test_sanitize_temperature_normal(self, climate_entity):
        """Test temperature sanitization with normal temperature."""
        result = climate_entity._sanitize_temperature(72.0)
        assert result == 72.0

    def test_sanitize_temperature_preset_max_cool(self, climate_entity):
        """Test temperature sanitization with max cool preset."""
        result = climate_entity._sanitize_temperature(-1)
        assert result is None

    def test_sanitize_temperature_preset_max_heat(self, climate_entity):
        """Test temperature sanitization with max heat preset."""
        result = climate_entity._sanitize_temperature(999)
        assert result is None

    def test_determine_hvac_mode_active(self, climate_entity):
        """Test HVAC mode determination for active status."""
        result = climate_entity._determine_hvac_mode("active")
        assert result == HVACMode.HEAT_COOL

    def test_determine_hvac_mode_standby(self, climate_entity):
        """Test HVAC mode determination for standby status."""
        result = climate_entity._determine_hvac_mode("standby")
        assert result == HVACMode.OFF

    def test_determine_hvac_mode_other(self, climate_entity):
        """Test HVAC mode determination for other status."""
        result = climate_entity._determine_hvac_mode("other_status")
        assert result == HVACMode.OFF

    def test_determine_preset_mode_max_cool(self, climate_entity):
        """Test preset mode determination for max cool."""
        result = climate_entity._determine_preset_mode(-1)
        assert result == PRESET_MAX_COOL

    def test_determine_preset_mode_max_heat(self, climate_entity):
        """Test preset mode determination for max heat."""
        result = climate_entity._determine_preset_mode(999)
        assert result == PRESET_MAX_HEAT

    def test_determine_preset_mode_normal(self, climate_entity):
        """Test preset mode determination for normal temperature."""
        result = climate_entity._determine_preset_mode(22.0)
        assert result == PRESET_NONE
...
class TestClimateSetup:
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_empty_data(self, hass: HomeAssistant):
